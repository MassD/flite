...
let email a j pl = 
  ...
  match html with 
    | Some h ->
      let subj = Printf.sprintf "Flite: %s to %s, %s-%s, back on %s-%s, last checked on %s" j.dep_ap j.arr_ap j.dep_mo j.dep_dy j.ret_mo j.ret_dy (Utils.string_of_utime j.last_fsed) in
